...
class DataParserService {
	...
	def parseNorcData(records, response) {
		/*
		def fileName = 'transmissionX_1_14APR11.xml'
		def inputFile = new File(fileName)
		if (!inputFile) {
			println "File ${fileName} not found. Stopping!"
			return
		}
		def xmlSlurper = new XmlSlurper()
		*/
		
		def checkSerial = null
		def dateTimeString = ""
		def now = new Date()
		...
		/* def records = xmlSlurper.parse(inputFile)
		if (records) {
			println "File loaded and parsed. ${now}"
		}
		else {
			println "File ${fileName} could not be parsed. Stopping!"
			return
		}*/
		
		/*
		   Primary XML Elements:
		   <HH_BATCH1>
		   <LOWCONSENT_BATCH1>
		   <LOWQUEX1_BATCH1>
		   <SCREENER_BATCH1>
		*/
		...
		def norcHhBatch = null
		def saveCount = 0
		// println "Parsing HH_BATCH1"
		records?.HH_BATCH1?.each{ c ->
			...
			checkSerial = c.Respondent_Serial.toString()
			norcHhBatch = NorcHhBatch.findByRespondentSerial(checkSerial)
			if (!norcHhBatch) {
			...
			} else if (norcHhBatch.save(flush:true)) {
				saveCount++
			}
			else {
		...
		println "    Done! ${saveCount} records saved to norcHhBatch"
		// end HH_BATCH1
		
		def norcLowConsentBatch = null
		saveCount = 0
		println "Parsing LOWCONSENT_BATCH1"
		records?.LOWCONSENT_BATCH1?.each{ c ->
			checkSerial = c.Respondent_Serial.toString()
			norcLowConsentBatch = NorcLowConsentBatch.findByRespondentSerial(checkSerial)
			...
			} else if (norcLowConsentBatch.save(flush:true)) {
				saveCount++
			}
			else {
		...
		println "    Done! ${saveCount} records saved to norcLowConsentBatch"
		// end LOWCONSENT_BATCH1
		
		def norcLowQuex1Batch = null 
		saveCount = 0
		println "Parsing LOWQUEX1_BATCH1"
		records?.LOWQUEX1_BATCH1?.each{ c ->
			checkSerial = c.Respondent_Serial.toString()
			norcLowQuex1Batch = NorcLowQuex1Batch.findByRespondentSerial(checkSerial)
			...
			} else if (norcLowQuex1Batch.save(flush:true)) {
				saveCount++
			}
			else {
		...
		def norcScreenerBatch = null
		saveCount = 0
		println "Parsing SCREENER_BATCH1"
		records?.SCREENER_BATCH1?.each{ c ->
			checkSerial = c.Respondent_Serial.toString()
			norcScreenerBatch = NorcScreenerBatch.findByRespondentSerial(checkSerial)
			...
			} else if (norcScreenerBatch.save(flush:true)) {
				saveCount++
			}
			else {
		...
		println "    Done! ${saveCount} records saved to norcScreenerBatch"
		// end SCREENER_BATCH1
